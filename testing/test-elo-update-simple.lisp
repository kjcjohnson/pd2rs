...
(defun elo (winnum losenum)
  "Is given the winning and losing elos and returns a two element list
	consisting of the new elos"
  (list (+ winnum (* *k-factor* (- 1 (expected-win winnum losenum)))) 
	(+ losenum (* *k-factor* (- 0 (expected-win losenum winnum))))))
...
(defun elo-input (inteam)
  ...
  (if inteam
      (team-elo inteam)
      *init-elo*))
(defun update-elo (teams games)
  "Uses the matches to update the teams' elos
	if the team dosn't exist it adds it to teams
	returns the teamlist"
  (loop for g in games do (let* ((t1 (find (game-winningid g) teams :test #'match-id))
				   (t2 (find (game-losingid  m) teams :test #'match-id))
				   (data (elo (elo-input t1) (elo-input t2))))
			      (if t1
				  (setf (team-elo t1) (car data))
				  (setf teams (cons (make-team :teamid (game-winningid g)
							       :elo (car data)
							       :rank 0
							       :timestamp "0") teams)))
			      (if t2
				  (setf (team-elo t2) (cadr data))
				  (setf teams (cons (make-team :teamid (game-losingid g)
							       :elo (cadr data)
							       :rank 0
							       :timestamp "0") teams))))
...
(defun calc-elos ()
  ...
  (loop named looper doing
	 ...
	 (setf teams (update-elo teams games))
					;sort by elo
